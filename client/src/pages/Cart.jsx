...
export default function Cart() {
    const { products, navigate, cartCount, cartItems, totalCartAmount, removeCart, updateCartItems, axios, user, setCartItems } = useContext(AppContext)
    //state to store products available in cart
    const [cartArray, setCartArray] = useState([])

    //state to store shipped  available address in cart
            ...
            if (paymentOption === "COD") {
                const { data } = await axios.post("/api/order/cod", {
                    items: cartArray.map((item) => ({
                        product: item._id,
                        quantity: item.quantity      // quantity: item._id
                    })),
    ...
    return products.length > 0 && cartItems ? (
                            ...
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        <p>Qty:</p>
                                        <select onChange={(e) => updateCartItems(product._id, Number(e.target.value))} value={cartItems[product._id] || 1} className='outline-none'>
                                            {Array(cartItems[product._id] > 9 ? cartItems[product._id] : 5).fill('').map((_, index) => (
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        <p className="text-gray-500">
                            {
                                selectedAddress ? `${selectedAddress.street},${selectedAddress.city},${selectedAddress.state},${selectedAddress.country}` : "No Address !"}
                        </p>
        ...
        </div >) : <h1>No Have Any Item In cart</h1>
}
